...
```{r}
#| label: join-population-regions-q9
...
**Comments:** All the data is not available in `cases_raw` yet because the CDC automatically limits the results to 1000 observations. We need to use the `$limit` parameter to expand the number of results.
...
# check that response has 200 status code
if (response$status_code == 200) {
  ...
  print(paste0("Unsuccessful response: ", response))
}
...
cases |> 
  ...
  arrange(year, month) |>
  kable(col.names=c("Month", "Year", "Total Cases"))
```
...
if (response$status_code == 200) {
  ...
  deaths <- response |>
    resp_body_json(simplifyVector = TRUE) |>
    # select state, date, deaths columns
    select(state,
           date=end_date,
           deaths=covid_19_deaths) |>
    # convert to date and numeric types
    mutate(date=as.Date(date), deaths=as.numeric(deaths)) |>
    # filter for states in previous data
